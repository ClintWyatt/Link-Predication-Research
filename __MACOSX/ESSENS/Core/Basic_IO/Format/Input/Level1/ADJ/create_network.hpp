	 ...
	 for(int i=0;i<b->size();i++)
     {
        ...
        // printf("%d %d \n", node1, node2);
		 ...
		 if(current_node!=node1)
         {
			...
			{
				 //Update Rows
				 X->push_back(AList); //Add row and neighbors to network. Pushes back information added after the previous loop
			 ...
			 //Set to next node
			 current_node=node1;
			 AList.Row=current_node;//setting the row equal to current_node (node1)
		 } //end of if
